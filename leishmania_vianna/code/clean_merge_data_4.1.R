...
infection.data <- reservoir.data[,c(1,16:24)]
infection.data$sum <- rowSums(infection.data[,2:10])
...
leish.status <- infection.data[,c(1,12)] #my rewritten code added extra rows, prob bc of the NA replace part so I just delete them here
...
leishmania_ranges <- leishmania_ranges[c(2,5,8,11,12,13),]
vianna_genus_range <- leishmania_ranges %>% summarise()
st_write(vianna_genus_range, "../raw data/leishmania_distributions/vianna_subgenus_range.shp")
...
total_trait_data <- rbind(positive_hosts, negative_hosts) #now down to 1470 hosts b/c removed 153 hosts
...
na.df$percent_coverage <- 1-na.df$NAlength/1460

## take columns with more than 1372 non-NA entries ((1314  = 90% of the data (so 90% can be missing)))
which.in = with(na.df, Variable[which(NAlength < 1314)])
...
cor.matrix <- cor(trait_data_small[,c(4:44, 53, 59:65)], use="pairwise.complete.obs") #make full correlation matrix
cor.matrix[is.na(cor.matrix)] <- 0 # change NA to 0 to apply cutoff
high.cor <- findCorrelation(cor.matrix, cutoff=0.7, exact = TRUE, names=TRUE)

#[1] "mean_tree_cover"          "SexualMaturityAge_d"      "hurs_mean"                "mean_canopy_height"      
#[5] "bio12_annual_precip"      "bio15_precip_seasonality" "cmi_mean"                 "bio16_precip_wettest_qt" 
#[9] "bio11_temp_coldest_qt"    "bio1_mean_annual_temp"    "AdultForearmLen_mm"       "AdultHeadBodyLen_mm"     
#[13] "GR_MidRangeLong_dd"       "GR_MinLat_dd"             "GR_MaxLat_dd"             "AdultBodyMass_g"         
#[17] "mean_humanpop_km2"
...
####################### Round Five: remove traits and species with low coverage, transform variables, remove wild hosts
####################### 

#subset down to wild hosts and remove domestic column & trophic level y
trait_data <- subset(trait_data, domestic == 0); trait_data <- trait_data[,-48]

trait_data <- trait_data[,-c(48:60)] #drop order

trait_data_small <- trait_data[,c(17, 1:16, 18:57)]
...
trait_data_small$pubmed.count <- 1/trait_data_small$pubmed.count #needed more extreme transformation
...
pubmed_data <- trait_data_small[,c(41,2:40, 42:57)]
...
#temp <- read.csv("../raw data/ACL_GEE_FINAL/mean_monthly_temp_2009to2019.csv")
#temp.agg <- aggregate(by=list(temp$BINOMIAL), x=temp$mean, FUN=mean) #take mean for each species
#names(temp.agg) <- c("MSW05_Binomial",'mean_temp_C')

##precip <- read.csv("../raw data/ACL_GEE_FINAL/range_mean_total_monthly_precip_2009to2019.csv")
#precip.agg <- aggregate(by=list(precip$BINOMIAL), x=precip$mean, FUN=mean) #take mean for each species
#names(precip.agg) <- c("MSW05_Binomial",'mean_rainfall_mm')

#vpd <- read.csv("../raw data/ACL_GEE_FINAL/mean_monthly_vpd_2009_to_2019.csv")
#vpd.agg <- aggregate(by=list(vpd$BINOMIAL), x=vpd$mean, FUN=mean) #take mean for each species
#names(vpd.agg) <- c("MSW05_Binomial",'mean_vpd')